...
function getKingdomStrength(kingdomId) {
  ...
  for (const vid of kingdom.villageIds) {
    const village = getVillage(vid);
    if (village) {
      total += village.troops.cityGuards * 1 + village.troops.spearmen * 2 + village.troops.archers * 2 + village.troops.mountedArcher * 3 + (village.troops.heavyKnight ?? 0) * 5 + (village.troops.shieldSoldier ?? 0) * 4 + (village.troops.samurai ?? 0) * 7 + (village.troops.mercenaryLancer ?? 0) * 6 + (village.troops.legionary ?? 0) * 6 + (village.troops.cavalryLancerElite ?? 0) * 8 + (village.troops.mercenaryRoman ?? 0) * 7 + (village.troops.mercenaryTemplar ?? 0) * 9 + (village.troops.mercenarySaracen ?? 0) * 6 + (village.troops.mercenarySparta ?? 0) * 8 + (village.troops.mercenaryViking ?? 0) * 8;
    }
  }
  return total;
}
...
function consumeSoldierFoodFromGranary(village) {
  ...
  const soldiers = village.troops.cityGuards + village.troops.spearmen + village.troops.archers + village.troops.mountedArcher + (village.troops.heavyKnight ?? 0) + (village.troops.shieldSoldier ?? 0) + (village.troops.samurai ?? 0) + (village.troops.mercenaryLancer ?? 0) + (village.troops.legionary ?? 0) + (village.troops.cavalryLancerElite ?? 0) + (village.troops.mercenaryRoman ?? 0) + (village.troops.mercenaryTemplar ?? 0) + (village.troops.mercenarySaracen ?? 0) + (village.troops.mercenarySparta ?? 0) + (village.troops.mercenaryViking ?? 0);
  if (soldiers === 0) {
...
init_notify();
init_bandit();
var ELITE_TYPES = /* @__PURE__ */ new Set(["samurai", "mercenaryLancer", "legionary", "cavalryLancerElite", "mercenaryRoman", "mercenaryTemplar", "mercenarySaracen", "mercenarySparta", "mercenaryViking"]);
var RECRUIT_COSTS = {
  cityGuards: 8,
  spearmen: 12,
  archers: 12,
...
function recruitTroop(village, type, count = 1) {
  ...
  const availableWorkers = village.population - village.troops.cityGuards - village.troops.spearmen - village.troops.archers - village.troops.mountedArcher - (village.troops.heavyKnight ?? 0) - (village.troops.shieldSoldier ?? 0) - (village.troops.samurai ?? 0) - (village.troops.mercenaryLancer ?? 0) - (village.troops.legionary ?? 0) - (village.troops.cavalryLancerElite ?? 0) - (village.troops.mercenaryRoman ?? 0) - (village.troops.mercenaryTemplar ?? 0) - (village.troops.mercenarySaracen ?? 0) - (village.troops.mercenarySparta ?? 0) - (village.troops.mercenaryViking ?? 0) - village.workers.farmers - village.workers.workers;
...
function tickWages(village) {
  ...
  const mr = village.troops.mercenaryRoman ?? 0;
  const mt = village.troops.mercenaryTemplar ?? 0;
  const msr = village.troops.mercenarySaracen ?? 0;
  const msp = village.troops.mercenarySparta ?? 0;
  const mv = village.troops.mercenaryViking ?? 0;
  const totalWages = village.troops.cityGuards * TROOP_WAGES.cityGuards + village.troops.spearmen * TROOP_WAGES.spearmen + village.troops.archers * TROOP_WAGES.archers + village.troops.mountedArcher * TROOP_WAGES.mountedArcher + hk * TROOP_WAGES.heavyKnight + ss * TROOP_WAGES.shieldSoldier + sa * TROOP_WAGES.samurai + ml * TROOP_WAGES.mercenaryLancer + le * TROOP_WAGES.legionary + cle * TROOP_WAGES.cavalryLancerElite + mr * TROOP_WAGES.mercenaryRoman + mt * TROOP_WAGES.mercenaryTemplar + msr * TROOP_WAGES.mercenarySaracen + msp * TROOP_WAGES.mercenarySparta + mv * TROOP_WAGES.mercenaryViking;
  if (totalWages === 0) {
...
function handleDesertion(village) {
  const deserters = {};
  const keys = ["cityGuards", "spearmen", "archers", "mountedArcher", "heavyKnight", "shieldSoldier", "samurai", "mercenaryLancer", "legionary", "cavalryLancerElite", "mercenaryRoman", "mercenaryTemplar", "mercenarySaracen", "mercenarySparta", "mercenaryViking"];
  let totalDeserters = 0;
...
var TROOP_TOKEN_MAP = {
  ...
  "kingdoms:mercenary_roman_token": { troopType: "mercenaryRoman", entityId: "kingdoms:mercenary_roman", label: "Roman Swordsman" },
  "kingdoms:mercenary_templar_token": { troopType: "mercenaryTemplar", entityId: "kingdoms:mercenary_templar", label: "Templar Knight" },
  "kingdoms:mercenary_saracen_token": { troopType: "mercenarySaracen", entityId: "kingdoms:mercenary_saracen", label: "Saracen Warrior" },
  "kingdoms:mercenary_sparta_token": { troopType: "mercenarySparta", entityId: "kingdoms:mercenary_sparta", label: "Sparta Elite" },
  "kingdoms:mercenary_viking_token": { troopType: "mercenaryViking", entityId: "kingdoms:mercenary_viking", label: "Viking Axeman" }
};
...
function pickupTroops(player, village, pickup) {
  ...
  if (pickup.mountedArcher > village.troops.mountedArcher) {
    notifyPlayer(player.name, `\xA7cNot enough Mounted Archers (have ${village.troops.mountedArcher}).`);
    return false;
  }
  ...
  if (pickup.shieldSoldier > (village.troops.shieldSoldier ?? 0)) {
    notifyPlayer(player.name, `\xA7cNot enough Shield Soldiers (have ${village.troops.shieldSoldier ?? 0}).`);
    return false;
  }
  ...
  if (pickup.cavalryLancerElite > (village.troops.cavalryLancerElite ?? 0)) {
...
function releaseTroops(player, usedItemId) {
  ...
  for (const [itemId, count] of Object.entries(found)) {
    const info2 = TROOP_TOKEN_MAP[itemId];
    if (!info2) continue;
    let spawned = 0;
    for (let n = 0; n < count; n++) {
      try {
        ...
        let entity;
        if (MOUNTED_ENTITIES.has(info2.entityId)) {
          entity = spawnMountedUnit(dim, info2.entityId, offset);
        } else {
          entity = dim.spawnEntity(info2.entityId, offset);
        }
        entity.nameTag = `${player.name}'s ${info2.label}`;
    ...
    if (spawned > 0) {
      ...
      parts.push(`${spawned} ${info2.label}`);
    }
  }
  if (parts.length === 0) {
...
function getTreasuryReport(village) {
  const wages = { cityGuards: 2, spearmen: 3, archers: 3, mountedArcher: 5, heavyKnight: 8, shieldSoldier: 6, samurai: 12, mercenaryLancer: 10, legionary: 10, cavalryLancerElite: 14, mercenaryRoman: 12, mercenaryTemplar: 15, mercenarySaracen: 10, mercenarySparta: 14, mercenaryViking: 13 };
  const dailyWages = (village.troops.cityGuards * wages.cityGuards + village.troops.spearmen * wages.spearmen + village.troops.archers * wages.archers + village.troops.mountedArcher * wages.mountedArcher + (village.troops.heavyKnight ?? 0) * wages.heavyKnight + (village.troops.samurai ?? 0) * wages.samurai + (village.troops.shieldSoldier ?? 0) * wages.shieldSoldier + (village.troops.mercenaryLancer ?? 0) * wages.mercenaryLancer + (village.troops.legionary ?? 0) * wages.legionary + (village.troops.cavalryLancerElite ?? 0) * wages.cavalryLancerElite + (village.troops.mercenaryRoman ?? 0) * wages.mercenaryRoman + (village.troops.mercenaryTemplar ?? 0) * wages.mercenaryTemplar + (village.troops.mercenarySaracen ?? 0) * wages.mercenarySaracen + (village.troops.mercenarySparta ?? 0) * wages.mercenarySparta + (village.troops.mercenaryViking ?? 0) * wages.mercenaryViking) / WAGE_INTERVAL_DAYS;
  return [
...
function upgradeWeapons(player, villageId) {
  ...
  const totalSoldiers = village.troops.cityGuards + village.troops.spearmen + village.troops.archers + village.troops.mountedArcher + (village.troops.heavyKnight ?? 0) + (village.troops.shieldSoldier ?? 0) + (village.troops.samurai ?? 0) + (village.troops.mercenaryLancer ?? 0) + (village.troops.legionary ?? 0) + (village.troops.cavalryLancerElite ?? 0);
...
function getBlacksmithSummary(village) {
  ...
  const soldiers = village.troops.cityGuards + village.troops.spearmen + village.troops.archers + village.troops.mountedArcher + (village.troops.heavyKnight ?? 0) + (village.troops.shieldSoldier ?? 0) + (village.troops.samurai ?? 0) + (village.troops.mercenaryLancer ?? 0) + (village.troops.legionary ?? 0) + (village.troops.cavalryLancerElite ?? 0);
  const rs = village.resourceStorage;
  return [
...
init_notify();
init_playerSettings();
var TROOP_TYPES = ["cityGuards", "spearmen", "archers", "mountedArcher", "heavyKnight", "shieldSoldier", "samurai", "mercenaryLancer", "legionary", "cavalryLancerElite", "mercenaryRoman", "mercenaryTemplar", "mercenarySaracen", "mercenarySparta", "mercenaryViking"];
...
function handlePopulationDecline(village) {
  if (village.population > 1 && Math.random() < MORTALITY_CHANCE) {
    ...
    if (village.population < totalSoldiers + village.workers.farmers + village.workers.workers) {
      ...
      else if ((village.troops.shieldSoldier ?? 0) > 0) village.troops.shieldSoldier--;
      ...
      else if ((village.troops.cavalryLancerElite ?? 0) > 0) village.troops.cavalryLancerElite--;
      else if ((village.troops.mercenaryRoman ?? 0) > 0) village.troops.mercenaryRoman--;
      else if ((village.troops.mercenaryTemplar ?? 0) > 0) village.troops.mercenaryTemplar--;
      else if ((village.troops.mercenarySaracen ?? 0) > 0) village.troops.mercenarySaracen--;
      else if ((village.troops.mercenarySparta ?? 0) > 0) village.troops.mercenarySparta--;
      else if ((village.troops.mercenaryViking ?? 0) > 0) village.troops.mercenaryViking--;
...
var TRAINING_COSTS = {
  ...
  mercenaryRoman: { emeralds: 22, iron: 14, gold: 8, diamonds: 5 },
  mercenaryTemplar: { emeralds: 28, iron: 16, gold: 10, diamonds: 7 },
  mercenarySaracen: { emeralds: 20, iron: 12, gold: 7, diamonds: 4 },
  mercenarySparta: { emeralds: 25, iron: 15, gold: 9, diamonds: 6 },
...
var TROOP_LABELS = {
  ...
  mercenaryRoman: "Roman Swordsman",
  mercenaryTemplar: "Templar Knight",
  mercenarySaracen: "Saracen Warrior",
  mercenarySparta: "Sparta Elite",
  mercenaryViking: "Viking Axeman"
};
var ELITE_TROOP_TYPES = ["samurai", "mercenaryLancer", "legionary", "cavalryLancerElite", "mercenaryRoman", "mercenaryTemplar", "mercenarySaracen", "mercenarySparta", "mercenaryViking"];
...
var TROOP_ENTITY_MAP = {
  ...
  mercenaryLancer: "kingdoms:mercenary_lancer",
  legionary: "kingdoms:legionary",
  shieldSoldier: "kingdoms:shield_soldier",
  cavalryLancerElite: "kingdoms:cavalry_lancer_elite",
  mercenaryRoman: "kingdoms:mercenary_roman",
  mercenaryTemplar: "kingdoms:mercenary_templar",
  mercenarySaracen: "kingdoms:mercenary_saracen",
  mercenarySparta: "kingdoms:mercenary_sparta",
  mercenaryViking: "kingdoms:mercenary_viking"
};
var TROOP_PRIORITY = ["mercenaryTemplar", "cavalryLancerElite", "mercenarySparta", "mercenaryViking", "samurai", "mercenaryRoman", "mercenaryLancer", "legionary", "heavyKnight", "shieldSoldier", "spearmen", "archers", "mountedArcher", "mercenarySaracen", "cityGuards"];
...
function dispatchTroops(village, threatCount) {
  const totalBarracks = village.troops.cityGuards + village.troops.spearmen + village.troops.archers + village.troops.mountedArcher + (village.troops.heavyKnight ?? 0) + (village.troops.shieldSoldier ?? 0) + (village.troops.samurai ?? 0) + (village.troops.mercenaryLancer ?? 0) + (village.troops.legionary ?? 0) + (village.troops.cavalryLancerElite ?? 0) + (village.troops.mercenaryRoman ?? 0) + (village.troops.mercenaryTemplar ?? 0) + (village.troops.mercenarySaracen ?? 0) + (village.troops.mercenarySparta ?? 0) + (village.troops.mercenaryViking ?? 0);
...
function getBestAvailableTroopType(village) {
  const types = ["mercenaryTemplar", "cavalryLancerElite", "mercenarySparta", "mercenaryViking", "samurai", "mercenaryRoman", "legionary", "mercenaryLancer", "heavyKnight", "shieldSoldier", "mountedArcher", "spearmen", "mercenarySaracen", "archers", "cityGuards"];
  ...
  system6.runTimeout(() => {
    ...
    if (!world20.getDynamicProperty(starterKey)) {
      const starterItems = [
        ...
        { id: "minecraft:emerald", count: 60 }
      ];
      const tryGiveStarterKit = (attemptsLeft) => {
        const p = world20.getPlayers().find((pl) => pl.name === playerName);
        if (!p) return;
        const inv = p.getComponent(EntityInventoryComponent8.componentId);
        if (!inv?.container) {
          if (attemptsLeft > 0) system6.runTimeout(() => tryGiveStarterKit(attemptsLeft - 1), 40);
          return;
        }
        const container = inv.container;
        for (const { id, count } of starterItems) {
          try {
            const leftover = container.addItem(new ItemStack6(id, count));
            if (leftover) {
              p.dimension.spawnItem(leftover, p.location);
            }
          } catch {
            p.runCommandAsync(`give @s ${id} ${count}`).catch(() => {
            });
          }
        }
...
\xA77You received: \xA7f1 Town Hall\xA77, \xA7f1 Village Spawner\xA77, \xA7f60 Emeralds\xA77.
...
\xA77Select troop type and quantity`).button(`\u2694 Pick Up Troops (${t.cityGuards + t.spearmen + t.archers + t.mountedArcher + hk + ss + sm2 + ml2 + lg2 + cle + (t.mercenaryRoman ?? 0) + (t.mercenaryTemplar ?? 0) + (t.mercenarySaracen ?? 0) + (t.mercenarySparta ?? 0) + (t.mercenaryViking ?? 0)} stationed)`).button(carriedTotal > 0 ? `\u{1F3F9} Return Troops to Barracks (${carriedTotal} carried)` : "\u{1F3F9} Return Troops (none carried)").button(`\u2B06 Upgrade Barracks (${village.barracksLevel * 15} emeralds)`).button("\u{1F4DC} Formation Scroll\n\xA77Take a formation command scroll");
  const response = await form.show(player);
  if (response.canceled) return;
  switch (response.selection) {
    case 0:
    ...
    case 3:
      ...
      break;
    case 4:
      giveFormationScroll(player);
      break;
  }
}
function giveFormationScroll(player) {
  const inv = player.getComponent(EntityInventoryComponent8.componentId);
  if (!inv?.container) return;
  const container = inv.container;
  for (let i = 0; i < container.size; i++) {
    const slot = container.getItem(i);
    if (slot?.typeId === "kingdoms:formation_scroll") {
      notifyPlayer(player.name, "\xA7eYou already have a Formation Scroll.");
      return;
    }
  }
  for (let i = 0; i < container.size; i++) {
    if (!container.getItem(i)) {
      container.setItem(i, new ItemStack6("kingdoms:formation_scroll", 1));
      notifyPlayer(player.name, "\xA7a\u{1F4DC} Formation Scroll added to your inventory. Right-click to command your troops!");
      return;
    }
  }
...
\xA77${makeCostLine("heavyKnight")}`).button(`Shield Soldier
\xA77${makeCostLine("shieldSoldier")}`).button(eliteAvailable ? `\u2B50 Samurai
...
\xA77${makeCostLine("cavalryLancerElite")}`).button(eliteAvailable ? `\u2B50 Roman Swordsman
\xA77${makeCostLine("mercenaryRoman")}` : `\xA77Roman Swordsman (${eliteLockMsg})
\xA77${makeCostLine("mercenaryRoman")}`).button(eliteAvailable ? `\u2B50 Templar Knight
\xA77${makeCostLine("mercenaryTemplar")}` : `\xA77Templar Knight (${eliteLockMsg})
\xA77${makeCostLine("mercenaryTemplar")}`).button(eliteAvailable ? `\u2B50 Saracen Warrior
\xA77${makeCostLine("mercenarySaracen")}` : `\xA77Saracen Warrior (${eliteLockMsg})
\xA77${makeCostLine("mercenarySaracen")}`).button(eliteAvailable ? `\u2B50 Sparta Elite
\xA77${makeCostLine("mercenarySparta")}` : `\xA77Sparta Elite (${eliteLockMsg})
\xA77${makeCostLine("mercenarySparta")}`).button(eliteAvailable ? `\u2B50 Viking Axeman
\xA77${makeCostLine("mercenaryViking")}` : `\xA77Viking Axeman (${eliteLockMsg})
\xA77${makeCostLine("mercenaryViking")}`).button("Back");
  const response = await form.show(player);
  if (response.canceled || response.selection === 15) return;
